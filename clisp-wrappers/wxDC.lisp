...
(defmacro with-client-DC ((dc window) &body body)
  `(let (,dc)
    (unwind-protect
	 (progn
	   (setf ,dc (wxClientDC_Create ,window))
	   ,@body)
      (wxClientDC_Delete ,dc))))

(defmacro with-paint-DC ((dc window) &body body)
  `(let (,dc)
    (unwind-protect
	 (progn
	   (setf ,dc (wxPaintDC_Create ,window))
	   ,@body)
      (wxPaintDC_Delete ,dc))))

(defmacro with-screen-DC ((dc window) &body body)
  `(let (,dc)
    (unwind-protect
	 (progn
