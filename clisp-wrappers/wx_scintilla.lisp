...
(defpackage :wx_scintilla
  ...
  (:export
	...
	:SCNotification-text
	:SCNotification-length
	:SCNotification-linesAdded
	:SCNotification-message
	:SCNotification-wParam
	:SCNotification-lParam
	:SCNotification-line
	:SCNotification-foldLevelNow
	:SCNotification-foldLevelPrev
...
(ffi:def-c-struct CharacterRange
	(cpMin :type ffi:long)
	(cpMax :type ffi:long))

(ffi:def-c-struct TextRange
	(chrg :type CharacterRange)
	(lpstrText :type ffi:c-string))

(ffi:def-c-struct TextToFind
	(chrg :type CharacterRange)
	(lpstrText :type ffi:c-string)
	(chrgText :type CharacterRange))

(ffi:def-c-struct NotifyHeader
	(hwndFrom :type (ffi:c-pointer NIL))
	(idFrom :type ffi:uint)
	(code :type ffi:uint))

(ffi:def-c-struct SCNotification
	(nmhdr :type _NotifyHeader)
	(position :type ffi:int)
	(ch :type ffi:int)
	(modifiers :type ffi:int)
	(modificationType :type ffi:int)
	(text :type ffi:c-string)
	(length :type ffi:int)
	(linesAdded :type ffi:int)
	(message :type ffi:int)
	(wParam :type ffi:ulong)
	(lParam :type ffi:long)
	(line :type ffi:int)
	(foldLevelNow :type ffi:int)
	(foldLevelPrev :type ffi:int)
	(margin :type ffi:int)
	(listType :type ffi:int)
	(x :type ffi:int)
	(y :type ffi:int))
